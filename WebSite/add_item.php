...
$itemTagid = mysqli_real_escape_string($mysqli, filter_var($formData["item-tag"], FILTER_SANITIZE_STRING));
$itemKeyid = mysqli_real_escape_string($mysqli, filter_var($formData["item-key"], FILTER_SANITIZE_STRING));
$itemDescription = mysqli_real_escape_string($mysqli, filter_var($formData["item-description"], FILTER_SANITIZE_STRING));

if (isset($newimg)) {
    $query = "INSERT INTO tItems (tagid, keyid, description, image) VALUES ('$itemTagid', '$itemKeyid', '$itemDescription', '$newimg')";
} else {
    $query = "INSERT INTO tItems (tagid, keyid, description, image) VALUES ('$itemTagid', '$itemKeyid', '$itemDescription', DEFAULT(image))";
}

// Execute an INSERT query to add the new item to the databa
